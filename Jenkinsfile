                ...
                container('argocdcli') {
                   ...
                   withCredentials([string(credentialsId: 'argo-secret-token', variable: 'TOKEN')]) {
                    sh '''
                     ...
                     APP_NAME="guestbook"
                     #ARGOCD_TOKEN=$TOKEN ARGOCD_SERVER=$ARGOCD_SERVER argocd app sync $APP_NAME --force --insecure
                     ARGOCD_SERVER=$ARGOCD_SERVER argocd app sync $APP_NAME --force --insecure --auth-token $TOKEN
                    '''
                    }
